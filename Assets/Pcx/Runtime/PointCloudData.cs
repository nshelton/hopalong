...
namespace Pcx
{
    ...
    public sealed class PointCloudData :ScriptableObject
    {
        ...
        public void Generate(float a, float b, float c, Klak.Chromatics.CosineGradient gradient)
        {
            if ( _pointData == null)
                _pointData = new Point[100000];
            ...
            {
                Color col = gradient.Evaluate((float)i / 100000);

                float xx = y - Mathf.Sign(x) * Mathf.Sqrt(Mathf.Abs(b * x - c));
                float yy = a - x;
